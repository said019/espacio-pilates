...
**Motivo:** La dueña pidió agregar a esta pantalla los mensajes que el estudio ya está usando y que sean "100% configurables". Al auditar el código se encontró que de las 8 plantillas mostradas hoy, solo 4 controlan un mensaje real; las otras 4 son controles decorativos (editar su texto no le llega a ninguna alumna). Además faltan 5 plantillas reales que sí se envían todos los días.
...
| `class_reminder` | Sí | Push (WA bloqueado) | Mantener + etiquetar canal |
...
| `last_class_reminder` | No | Push (WA bloqueado) | Agregar |

Resultado: **10 plantillas** editables en total (5 actuales + 5 nuevas − 3 quitadas... son 8 actuales, se quitan 3, quedan 5, más 5 nuevas = 10), todas con efecto real verificado.
...
Eliminar las entradas `renewal_reminder`, `welcome`, `password_reset` (ningún call site las lee; verificado por grep exhaustivo de `templateKey:`). Eliminar también `renewal_reminder: "/app"` de `PUSH_TEMPLATE_URLS` (~línea 9880).
...
- Quitar las entradas `welcome`, `password_reset`, `renewal_reminder`.
- Agregar 5 entradas nuevas: `class_reminder_12h`, `class_reminder_30m`, `booking_waitlist`, `booking_waitlist_promoted`, `last_class_reminder` (label, icon, hint con variables reales tomadas de `DEFAULT_NOTIFICATION_TEMPLATES`/call sites).
...
- Verificación visual: las 10 plantillas se ven con su badge de canal correcto; las 3 quitadas ya no aparecen.

## 6. Archivos tocados
| Archivo | Cambio |
|---|---|
| `server/index.js` | push a `transfer_rejected`; quitar 3 claves muertas de `DEFAULT_NOTIFICATION_TEMPLATES` y `PUSH_TEMPLATE_URLS` |
| `src/pages/admin/settings/SettingsPage.tsx` | `NOTIFICATION_TEMPLATES` (+5/−3, campo `channels`); badges de canal; relabel del campo "Asunto" |
